...
namespace VehicleBehaviour.Trails
{
	...
	public class TrailEmitter : MonoBehaviour {
		...
		void Update () 
		{
			...
			if (!trailing && _wheel.isGrounded && (Mathf.Abs(hit.sidewaysSlip) > 0.9f || Mathf.Abs(hit.forwardSlip) > 0.98f)) 
			{
				trailing = true;
				NewTrail ();
			} else if (trailing && (!_wheel.isGrounded || (Mathf.Abs(hit.sidewaysSlip) < 0.5f && Mathf.Abs(hit.forwardSlip) < 0.98f))) 
			{
				trailing = false;
				EndTrail ();
			}
