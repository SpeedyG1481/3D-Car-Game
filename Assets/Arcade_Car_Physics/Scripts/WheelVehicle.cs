...
namespace VehicleBehaviour
{
    ...
    public class WheelVehicle : MonoBehaviour
    {
        ...
        [Header("Wheels")] [SerializeField] WheelCollider[] driveWheel;

        public WheelCollider[] DriveWheel
        {
            get { return driveWheel; }
        }
        ...
        void FixedUpdate()
        {
            ...
            else if (Mathf.Abs(speed) < 4 || Mathf.Sign(speed) == Mathf.Sign(_throttle))
            {
                ...
                {
                    wheel.motorTorque = _throttle * motorTorque.Evaluate(speed) * diffGearing / driveWheel.Length;
                }
            }
            else
            {
            ...
            if (drift && allowDrift)
            {
                ...
                if (_steering != 0)
                    driftForce *= _rb.mass * speed / 7f * _throttle * _steering / steerAngle;
                Vector3 driftTorque = transform.up * 0.1f * _steering / steerAngle;
